...
- **Atomic scene changes**: `scene_change_plan` now refuses to create a plan while the console contains errors. `apply_scene_change` rejects empty input and commands without proven Unity Undo coverage, uses an atomic stop-on-error batch, recognizes plain and multiline handler errors, and only verifies or saves after a successful apply. Broken references, console errors, and unrecognized verification results block saving.
- **Hierarchy summaries**: Summary mode now honors the requested scene, including for unqualified roots. Hierarchy compression recognizes current `&` references, respects tree depth, and preserves the final sibling marker.
...
- **Side-effect safety**: `uitk_file(read)` is classified as read-only, while `test_step`, playtest runs, `verify_after_change`, and `screenshot_baseline` are correctly classified as writes. `doctor` remains observational by default, but its stale-file cleanup is blocked on read-only endpoints. Other or unknown `uitk_file` actions remain writes, `auto_fix` is documented as a read-only suggestion generator, and screenshot baseline names can no longer be empty, path-like, or traversal strings.
- **Playtest verification resets**: `verify_after_change(restart_between=True)` now starts and resets Play Mode through observed state transitions. Start, stop, restart, cleanup, empty matches, and zero-test suites fail closed instead of producing a passing gate.
...
- **ClientSkills**: The shipped inventory is now 12 skills and 4 agents. The retired `unity-test-reviewer` agent and `test-authoring.md` policy are removed; transactional upgrade migrations delete only recognized, unmodified legacy files and preserve customized conflicts.
...
[Unreleased]: https://github.com/german-krasnikov/unity-biome-mcp/compare/v1.36.0...HEAD
