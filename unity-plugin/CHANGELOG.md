...
- **Modular Architecture Phase 2 — Self-Contained Module Pilot (PR-04):** `WatchModule` owns its registration metadata via declarative `SPEC_KWARGS` merged at import time. Instance-scoped `_send`/`_args` injection proves module isolation. C# `CommandRegistry.Entry` gains `MutatingArgsPolicy` delegate and `NotBatchable` bool; `uitk_file` hardcoded name-switch migrated to registration site. 36 policy-vector tests cover read/write, batch, and retry-safety for 5 representative commands. Invariant: adding a command requires only the module's registration — no central list edits
- **Modular Architecture Phase 2 — Reload Module Facade with Replaceable Algorithm (PR-04R):** `SyncHelper.IsMainAssemblyCompiling` is now an injectable predicate (the identified architectural seam). `SourcePatchModePolicy.RequestDisable()` delegates via `ISourcePatchReloadPort` instead of direct `SyncHelper.TriggerSync` call. Import-linter forbids diagnose/reload_ladder from importing sync.py. Invariant: Reload algorithm can be swapped via composition without edits to core, transport, Chat, or SourcePatch
- **Modular Architecture Phase 2 — Chat Off as Real Disconnection (PR-05):** Removed stale reflection probes in `ChatBackendProbe`/`ChatSettingsHook`; Chat assemblies now wire owned delegates via `[InitializeOnLoad]`. `ask_user` returns immediately when no Chat subscriber exists (no 300s hang). `search_context` throws `ProviderUnavailableException` (classified UNAVAILABLE, logged as warning) when Chat.CLI absent. Nine Chat-dependent test files moved from base to Chat.Tests.CLI/View; base asmdef no longer references Chat. Invariant: Chat Off/absent leaves MCP tools fully operational; no stale process, pending-ask leak, or false-positive error
- **Modular Architecture Phase 2 — SourcePatch Off/Absence Proof & Reload Boundary (PR-06):** `ForceUnreconciledForTests()` seam on `SourcePatchHost` enables contract tests. Two C# tests prove real lazy-reconciliation to Off and concurrent-sync epoch drift resolves to Recovery (not false-Off). Python boundary scan locks `SyncHelper.TriggerSync` out of SourcePatch control files except sanctioned adapter. Invariant: SourcePatch Off leaves normal writes and domain reload unintercepted; provider/algorithm swaps without core changes
- **DSL Conformance Oracle Strengthened (PR-07):** Strict error classification in DSL harness. Fixed ci-conformance.yml lane-generation to use `gen_lane_args.py`. Add zero-selection fail-closed guard. Documented reserved header fields (`NeedsPlaymode`, `ExpectSteps`, `ExpectFailed`, `SuiteOnly`) as parsed but unenforced. New `PreparedPlaytest` immutable container for data isolation. `ConsoleErrored` field serialized in receipt `ToJson` (additive, backward-compatible). Invariant: DSL conformance tests classify errors specifically; prepared playtest data is immutable; lane selection cannot silently produce empty set
- **Coverage Hotspots and Clone Detection (PR-08):** New `opencover_parse.py` (OpenCover XML → method-level cyclomatic complexity + coverage), `coverage_hotspots.py` (changed-aware top-20 report with scenario map), `cochange_report.py` (manual cochange analysis). Configure `jscpd` v5.1.2 with baseline-from-ref and fail-on-new-clones gate in ci-python.yml. Coverage-hotspots CI job reports to `docs/quality/` on master push. Invariant: new substantive code duplicates fail CI; coverage hotspots ranked by changed × complexity × uncovered

### Fixed

- **F7 — Honest Terminal Outcome for Playtest Verdict:** `PlaytestStepReceipt.Ok` now accounts for console errors detected after the step passes. Both text and JSON verdict consumers reject ABORTED runs, receipts with contradictory aggregate/step ledgers, and empty/malformed required receipts. Invariant: abort, console-error-on-passing-step, cleanup failure, and empty receipt are non-PASS in all formats (text/JSON) and all paths (sync/async start-poll)
- **F8 — Unified Read-Only Policy Across All Routes:** Remove `_INTERNAL` category filter from `WRITE_CMDS` that allowed `start_playtest` and `source_patch_write` to bypass read-only gate. Reclassify ping/get_version as explicit reads. Add `unknown_is_write` fail-closed fallback at both authorization call sites. Mirror `start_playtest` into `SCENE_STATE_NEUTRAL_WRITES` to match `run_playtest`. Invariant: read-only blocks mutating commands regardless of sync/async route, warmup state, or category
- **F1 — Player Profile Preflight Rejects Unsupported DSL Before Execution:** `PlayerProfilePreflight.Validate(ParseResult)` in engine-free Playtest.Core checks fatal parse errors, `@needs editmode` header, SETUP/TEARDOWN blocks, EXPECT_FAIL, compound WAIT_UNTIL AND/OR, ASSERT...TIMEOUT retry semantics, and empty main section. `PlayerPlaytestRunner.Run()` wraps load+parse in try/catch and delegates gate decision to preflight. Invariant: unsupported Player scripts are rejected with terminal receipt before any body action executes
- **F2 — Conservative Scene Cache Invalidation After Playtest Scenario:** Split `SCENE_STATE_NEUTRAL_WRITES` into two policies: "counts as blind write for advisory guard" (unchanged) vs. "can make caches stale" (new `is_scenario_terminal` predicate). `Middleware.invalidate_scene_caches()` uses `_scene_generation` counter to fence late in-flight background prefetches from re-poisoning cleared caches. Cover sync, async start/poll, suite, wire-exception, and poll-give-up paths. Invariant: a public read immediately after playtest scenario completion returns fresh scene state

### Changed

- **Architectural Boundaries Established:** Import-linter contracts mechanically prevent tools→server, tools→middleware, bridge→session, plugin_api→server imports. Static asmdef boundary checks for Playtest.Core (`noEngineReferences`) and SourcePatch (no provider refs). CI validates architectural integrity on every build
- **Hygiene:** Replaced blocking `.Result` with `await` in `CommandRouterAskUserTests` (test-only)

### Test Results

- **Modular architecture phase 2 suite:** All policy vectors green; plugin atomicity verified; chat/sourcepatch boundaries confirmed
- **DSL conformance:** Reserved field parsing confirmed; Player preflight gate validated
