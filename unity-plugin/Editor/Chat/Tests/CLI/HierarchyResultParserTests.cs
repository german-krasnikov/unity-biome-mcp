...
//   Root: "Name &Base62Ref"           (no connector prefix)
//   Depth 1: "│  └─ Name &Base62Ref" or "   └─ Name &Base62Ref"
...
namespace UnityMCP.Editor.Chat.Tests
{
    ...
    public class HierarchyResultParserTests : UnityMCP.Editor.Testing.UnityMcpTestBase
    {
        ...
        [Test]
        public void Parse_SingleRootNode_NameReferenceDepth()
        {
            var nodes = HierarchyResultParser.Parse("Main Camera &1");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("Main Camera", nodes[0].Name,      "Name stripped of reference");
            Assert.AreEqual("&1",          nodes[0].Reference, "Canonical reference preserved");
        ...
        {
            var nodes = HierarchyResultParser.Parse("Enemy NPC &2 !");
            Assert.AreEqual(1, nodes.Length);
        ...
        {
            var nodes = HierarchyResultParser.Parse("World &3 +12");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual(12, nodes[0].HiddenCount, "+12 must be parsed as HiddenCount");
        }

        [Test]
        public void Parse_InactiveNodeWithHiddenDescendants_AllFlagsParsed()
        {
            var nodes = HierarchyResultParser.Parse("Name &1 ! +5");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("Name", nodes[0].Name);
            Assert.AreEqual("&1", nodes[0].Reference);
            Assert.IsTrue(nodes[0].IsInactive);
            Assert.AreEqual(5, nodes[0].HiddenCount);
        }
        ...
        {
            // Single root's child (parent was last → 3 spaces).
            var nodes = HierarchyResultParser.Parse("   └─ Weapon &4");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual(1, nodes[0].Depth, "3-space prefix = depth 1");
            Assert.AreEqual("Weapon", nodes[0].Name);
            Assert.AreEqual("&4", nodes[0].Reference);
        }

        [Test]
        public void Parse_ChildOfNonLastRoot_Depth1()
        {
            // Child of non-last root (│   prefix).
            var nodes = HierarchyResultParser.Parse("│  ├─ Shield &5");
            Assert.AreEqual(1, nodes.Length);
        ...
        {
            var nodes = HierarchyResultParser.Parse("│  │  └─ Barrel &6");
            Assert.AreEqual(1, nodes.Length);
        ...
        public void Parse_DeepNesting_Depth5()
        {
            ...
            var nodes = HierarchyResultParser.Parse("│  │  │  │  │  └─ VeryDeep &7");
            Assert.AreEqual(1, nodes.Length);
        ...
        [Test]
        public void Parse_SceneHeaderContainingAmpersand_IsSceneHeaderTrue()
        {
            var nodes = HierarchyResultParser.Parse("[R&D Scene]");
            Assert.AreEqual(1, nodes.Length);
            Assert.IsTrue(nodes[0].IsSceneHeader);
            Assert.AreEqual("R&D Scene", nodes[0].SceneName);
        }

        [Test]
        public void Parse_BracketNamedObjectWithReference_IsNotSceneHeader()
        {
            var nodes = HierarchyResultParser.Parse("[Gameplay] &8");
            Assert.AreEqual(1, nodes.Length);
            Assert.IsFalse(nodes[0].IsSceneHeader,
                "A serializer node keeps its trailing reference and is not a scene header");
            Assert.AreEqual("[Gameplay]", nodes[0].Name);
            Assert.AreEqual("&8", nodes[0].Reference);
        }
        ...
        {
            var nodes = HierarchyResultParser.Parse(
                "Player [Health,Rigidbody] &a", parseComponents: true);
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("Player",            nodes[0].Name);
            Assert.AreEqual("Health,Rigidbody",  nodes[0].Components);
            Assert.AreEqual("&a",                nodes[0].Reference);
        }

        [Test]
        public void Parse_ComponentsDisabled_BracketSuffixRemainsPartOfName()
        {
            var nodes = HierarchyResultParser.Parse("Boss [Phase Two] &b");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("Boss [Phase Two]", nodes[0].Name,
                "Object names ending in brackets must remain intact by default");
            Assert.IsNull(nodes[0].Components);
        }
        ...
        public void Parse_CyrillicName_ParsedCorrectly()
        {
            ...
            var nodes = HierarchyResultParser.Parse("Главный Герой &Z !");
            Assert.AreEqual(1, nodes.Length);
        ...
        {
            // Display keeps the escaped slash raw; navigation uses the compact reference.
            var nodes = HierarchyResultParser.Parse("Day\\/Night &10");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("Day\\/Night", nodes[0].Name, "Escaped slash kept as-is");
            Assert.AreEqual("&10", nodes[0].Reference);
        }

        [Test]
        public void Parse_MixedCaseBase62Reference_Preserved()
        {
            var nodes = HierarchyResultParser.Parse("Base62 Target &aZ09");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("&aZ09", nodes[0].Reference);
        }

        [Test]
        public void Parse_V132FormatLegacyHexResult_StillReadable()
        {
            // Result text produced by v1.32 remains parseable during the transition.
            var nodes = HierarchyResultParser.Parse("Legacy Player $A1B2C3");
            Assert.AreEqual(1, nodes.Length);
            Assert.AreEqual("Legacy Player", nodes[0].Name);
            Assert.AreEqual("$A1B2C3", nodes[0].Reference);
        }

        [TestCase("Invalid &1_2")]
        [TestCase("Invalid &é")]
        [TestCase("Invalid $NOTHEX")]
        public void Parse_InvalidReferenceToken_SkipsLine(string input)
        {
            Assert.AreEqual(0, HierarchyResultParser.Parse(input).Length);
        }
        ...
        public void Parse_TruncatedMidLine_PartialLineSkipped()
        {
            ...
            // Partial line has no valid reference → TryParseNode returns false → skipped.
        ...
        public void Parse_ComplexScene_AllFieldsCorrect()
        {
            ...
            Assert.AreEqual("&1", nodes[1].Reference);
            ...
            Assert.AreEqual("&4",           nodes[4].Reference);
            ...
            Assert.AreEqual("&5", nodes[6].Reference);
        }
    }
}
