...
namespace UnityMCP.Editor.Chat
{
    internal static class RelayCommandResolver
    {
        ...
        ///   Non-local     → uvx --from &lt;git url pinned to plugin version&gt; unity-mcp-relay
        /// </summary>
        internal static (string cmd, string[] argv) Resolve()
        {
            if (InstallSourceDetector.Detect() != InstallSourceDetector.Source.Local)
            {
                ...
                return (uvx, new[] { "--from", url, "unity-mcp-relay" });
            }
            ...
            // instead of depending on the real Packages/com.unity-mcp.editor on disk.
        ...
        // "run --directory dir unity-mcp"). Reuse its venv/uv/fallback decision (no duplicated
        ...
        private static string[] RelayArgsFor(string[] serverArgs, string serverDir) =>
            serverArgs.Length > 0 && serverArgs[0] == "-m"
                ...
                : new[] { "run", "--directory", serverDir, "unity-mcp-relay" };
