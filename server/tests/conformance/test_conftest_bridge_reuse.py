...
The teardown dispatch (marker gate + bridge-reuse cleanup) used to open a
fresh `connect_bridge` on every single `live`+`conformance` test's teardown.
These tests pin the current behavior: cache one bridge for the whole
session, and reconnect only when the cached bridge is no longer usable
(closed/unreachable). No live Unity worker is involved — `connect_bridge`
and `ConformanceWorker` are faked.

Exercises `conf_conftest._teardown_live_conformance_item` directly (the
plain-async helper the autouse `_live_conformance_teardown` fixture calls)
via a fresh `asyncio.run()` per call — safe here because `_FakeBridge` holds
no real loop-bound asyncio primitives, unlike the live `_session_bridge`.
"""
...
def test_teardown_reuses_cached_bridge_across_dummy_items(monkeypatch):
    ...
    for _ in range(3):
        asyncio.run(conf_conftest._teardown_live_conformance_item(_dummy_live_conformance_item()))
