...
Port discovery chain: no explicit port embedded in config itself (server self-discovers via `~/.unity-mcp/ports/*.port`), but env var in config accelerates connection. The hardcoded `~/.claude/mcp.json` path is now a fallback only (used if config generation fails). This eliminates the need for manual setup in most cases.
